...
A hidden state $h_t$ is updated according to the input of timestamp $t$ and previous hidden state $h_{t-1}$. \QM{As for Vanilla RNNs, the hidden states are updated as a function:} 
\begin{equation} 
...
\begin{figure}[t]
	...
	\vspace{-5mm}
	\caption{\UC{Architectures considered in our experiment: A) RNN: the RNN layer is directly connected to the output layer; B) RNN-Dense: adds dense layers between the RNN layer and the output layer.}
	}
